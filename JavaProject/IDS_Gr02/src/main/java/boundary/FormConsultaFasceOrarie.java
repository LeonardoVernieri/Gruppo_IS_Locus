...
public class FormConsultaFasceOrarie extends JFrame {
    ...
    private static final Color BORDER_LIGHT   = new Color(0xE5E5E3);

    private static final Color BLUE_BG        = new Color(0xE6F1FB);
    private static final Color BLUE_BORDER    = new Color(0x85B7EB);
    private static final Color BLUE_TEXT      = new Color(0x0C447C);

    private static final Color GREEN_BG       = new Color(0xEAF3DE);
    private static final Color GREEN_TEXT     = new Color(0x3B6D11);
    private static final Color AMBER_BG       = new Color(0xFAEEDA);
    private static final Color AMBER_TEXT     = new Color(0x854F0B);
    private static final Color RED_BG         = new Color(0xFCEBEB);
    private static final Color RED_TEXT       = new Color(0xA32D2D);

    private static final Color TEXT_PRIMARY   = new Color(0x1A1A18);
    private static final Color TEXT_SECONDARY = new Color(0x6B6B68);
    private static final Color TEXT_TERTIARY  = new Color(0x9B9B98);

    private static final double SOGLIA_QUASI_PIENO = 0.20;

    // ── Stato ─────────────────────────────────────────────────────────────────
    private final GestoreSaleStudio gestore;
    private JComboBox<String> comboSale;
    private JPanel datePanelInner;
    private JScrollPane dateScrollPane;
    private JPanel fascePanelInner;
    private JScrollPane fasceScrollPane;
    private JLabel fasceHeaderLabel;
    private JLabel hintLabel;

    private final List<DateButton> dateButtons = new ArrayList<>();
    ...
    public FormConsultaFasceOrarie() {
        ...
        JLabel titolo = new JLabel("Consulta fasce orarie disponibili");
        ...
        JButton btnTorna = new JButton("Torna al menu");
        btnTorna.setAlignmentX(Component.LEFT_ALIGNMENT);
        btnTorna.setFocusPainted(false);
        btnTorna.setFont(btnTorna.getFont().deriveFont(13f));
        btnTorna.addActionListener(e -> {
            dispose();
            new FormStudente(); // sostituisci con il nome reale della tua classe
        });
        root.add(btnTorna);
    ...
    private void onSalaSelezionata() {
    ...
    private void aggiornaFasce() {
        ...
        Map<FasciaOraria, Integer> fasceDisponibili = new TreeMap<>(
                Comparator.comparing(f -> f.getOraInizio())
        );
        ...
        if (fasceDisponibili.isEmpty()) {
            ...
            // Card contenitore unica con divisori interni
            ...
            for (int i = 0; i < entries.size(); i++) {
                ...
                int totale = gestore.getNumPostazioni(entry.getKey(), dataSelezionata, nomeSala);
    ...
    private JPanel buildSlotRow(FasciaOraria fascia, int liberi, int totale) {
    ...
    private JLabel buildBadgePill(int liberi, int totale) {
        String testo;
        Color bg, fg;
    ...
    private JSeparator buildInternalDivider() {
    ...
    /** Bottone data con stato selected gestito internamente. */
    private static class DateButton extends JButton {
        private boolean selected = false;
        private final String giorno;
        private final String numero;

        DateButton(String giorno, int numero) {
            super();           // nessun testo: disegniamo tutto noi in paintComponent
            this.giorno = giorno;
        ...
        protected void paintComponent(Graphics g) {
            // NON chiamiamo super.paintComponent per evitare che Swing disegni
            // il testo del bottone (che sarebbe vuoto ma causerebbe artefatti).
    ...
    /** Card con bordo arrotondato e sfondo bianco. */
    ...
    /** Badge a pillola con colori semantici. */
